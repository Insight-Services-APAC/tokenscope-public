 ...
 * (one per (day, vendor) with positive cost), keyed by the vendor DISPLAY name
 * (`Claude` / `Copilot` / `Other`). We remap those to the chart-kit tool codes
 * (`claude-code` / `copilot-cli` / `other`) so ChartTrend's colorForKey paints
 * Claude = brand-hunger MAGENTA and Copilot = brand-vision blue (the fix for the
 * old StackedBars rendering Claude as harmony purple).
 *
...
export interface RegionalTrendRow {
  day: string
  /** Vendor DISPLAY name as the endpoint emits it: `Claude` | `Copilot` | `Other`. */
  key: string
  value: number
}
...
/** Endpoint display-name → chart-kit tool code + legend name. */
const VENDOR_META: Array<{ src: string; key: string; name: string }> = [
  { src: 'Claude', key: 'claude-code', name: 'Claude Code' },
  { src: 'Copilot', key: 'copilot-cli', name: 'GitHub Copilot' },
  { src: 'Other', key: 'other', name: 'Other' },
]
...
export function buildRegionalTrend(
  ...
  // Per-vendor day→value maps + the sorted set of ACTUAL days (keyed by src name).
  const byVendor = new Map<string, Map<string, number>>()
  const days = new Set<string>()
  for (const m of VENDOR_META) byVendor.set(m.src, new Map())
  ...
  const dailyTotals = actualDays.map((d) =>
    VENDOR_META.reduce((a, m) => a + (byVendor.get(m.src)?.get(d) ?? 0), 0),
  )

  const vendorTotal = (src: string): number => {
    let t = 0
    for (const v of byVendor.get(src)?.values() ?? []) t += v
    return t
  }
  // Claude + Copilot are the requested pair (always shown); Other only when it
  // carries spend — a flat zero "Other" line would be noise.
  const activeVendors = VENDOR_META.filter((m) => m.src !== 'Other' || vendorTotal('Other') > 0)
  ...
  const series: TrendSeries[] = activeVendors.map((m) => {
    const vm = byVendor.get(m.src) ?? new Map<string, number>()
