...
maintainers, and operators. For *why* the system is shaped this way see the
design docs (`docs/design/`) and ADRs (`docs/decisions/`); this wiki is *what is
actually running*.
...
> designed but **not built** — each page marks *Planned* items explicitly.
...
governs by *financial gravity* — additive budgets, velocity limits, a spill
...
- **[Background Workers](Background-Workers.md)** — the external-cron scheduler + the 25 workers.
...
- **[Data Protection](Data-Protection.md)** — data classification, PII, what is *not* collected, retention, residency.
