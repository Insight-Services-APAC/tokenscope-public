...
- **A single-flight `409` is a skipped tick, not a failed execution.** When a worker has a backlog its run can outlast its own cron interval, so the next tick hits the lock and gets `409 worker-already-running`. The trigger exits `0` and logs `SKIPPED (already running)`, so the Container Apps job execution records `Succeeded` and a healthy catch-up stays distinguishable from an outage in the job history. Every other non-2xx exits `1`, including any future `409` that is not the single-flight lock: the trigger matches the ProblemDetails type, not the bare status (`scripts/lib/dispatch-outcome.mjs`).
...
| `session-gc` | Close idle devices: set `ts_actual_end` once both `ts_expected_end` (renewed by every `/bearer` mint) and last mint + 90 days have passed. Closes in committed chunks of 500 with their audit events, skipping rows a renewal holds, within a 60 s budget (the next run continues a backlog). The scan uses a partial index over open devices (mig 0142). Ending a device revokes its bound credentials (mig 0141) | `0 2 * * *` |
| `soft-purge` | Apply 12-month retention — clear PII from ENDED devices enrolled over 12 months ago, keep FK-load-bearing columns | `0 3 * * *` |
...
| `read-path-health` | Alert admins when the OTel read path (`azure-monitor-read`) has silently stalled: a zero-write streak while the DCR ingest pipeline received rows the joiner did not land (`RowsReceived_Count`, read over the Azure Monitor METRICS API — independent of the reader's own output). When that metric cannot be read it falls back to the bearer keep-alive gate (errs toward paging). Also fires on ALL-FAULT (latest run errored on every session) and NO-SUCCESS (no successful run in 30 min). Auto-resolves once rows land | `*/15 * * * *` |
