...
| R10 | **Copilot flat-seat showback does not populate for an App-mode enterprise** — the enterprise seats pull (`listSeats()` / `listSeatsWithDiagnostics()`) always presents a Bearer PAT header, and an App-constructed client holds no PAT | **Functional gap, not a leak.** The call **fails loud with a 401**, isolated to that scope — no credential crosses a boundary and no wrong number is written. Chargeback is unaffected (Copilot bills pooled per cost-centre and is firewalled out of every chargeback view by migration `0085`); what is missing is the flat-seat **showback** row. It becomes live the moment an enterprise flips from PAT to the GitHub App, which is the direction of travel | Route the seats pull through the per-org App path `github.ts` / `github-identity.ts` already use |
...
- [ ] **`FORCE ROW LEVEL SECURITY` + a non-owner DB role** — the control that makes
      ...
      sequence is: universal GUC coverage → policies for the 35 tables still
      uncovered → a DBA-provisioned non-owner role (Azure PG Flexible Server has no
      ARM/Bicep resource for a native login, so this runs out of band) → a
      `database-url-app` KV secret + `secretRef` + an `infra.yml` apply → `FORCE`
      **per table**, sandbox → dev → staging → production, watching for
      empty-result regressions at each step. A CI guard
      (`scripts/check-handler-rls-context.mjs`) now pins the handler count at 47 so
      the debt cannot grow while this waits.
