...
describe('baselineGrants — pinned to WHO_SEES_WHAT_BASELINE (5 personas)', () => {
  it('the persona set is exactly these five — a silent shrink must FAIL, not vanish', () => {
    /*
     * Every case below is generated by iterating REPORT_VISIBILITY_PERSONAS, so
     * deleting a persona deletes its tests instead of failing them. That is how
     * the `global-finops` row left the matrix without a single red test. Pin the
     * set explicitly: adding or removing a persona now has to be a deliberate
     * edit here.
     */
    expect(REPORT_VISIBILITY_PERSONAS.map((p) => p.key).sort()).toEqual([
      'admin',
      'cost-centre-owner',
      'developer',
      'manager',
...
describe('baselineGrants — byte-identical to the retired standard grants for non-elevated roles', () => {
  ...
  it('admin (region admin): region-bound, identical to manager — NOT cross-region', () => {
    ...
    // full-company default is only for the org-wide roles (platform-admin /
  ...
  it('platform-admin: full company access AT BASELINE, no grant required (PO decision 2026-08-13)', () => {
  ...
  it('platform-admin: ownership does not change the full-access floor (it is already maximal)', () => {
...
describe('effectiveReportGrants — pinned to WHO_SEES_WHAT_ELEVATED (both permissions, 5 personas)', () => {
  for (const persona of REPORT_VISIBILITY_PERSONAS) {
