...
// below. This module's job is only to mint the password and the URL built from
// it; NOTHING here turns the feature on (see container-app.bicep's four flags,
// all default false).
//
// GENERATED, NEVER TYPED. main.bicep passes newGuid() so the value is created by
// the deployment and read only by Key Vault and the container — no human, no
// log, no repository. Empty = NO-OP per the SAFETY CONTRACT above, so an
// ordinary apply never rewrites (and therefore never ROTATES) an existing
// password: rotation is an explicit `writeAppDbPassword=true` apply, because
// rotating under a live app would leave draining replicas holding a password the
// role no longer has.
@description('App-role database password. Generated by main.bicep (newGuid); empty = NO-OP, leaving any existing app-db-password + database-url-app secrets untouched.')
@secure()
...
var canBuildAppDatabaseUrl = !empty(appDbPassword) && !empty(pgServerFqdn)

// THE ROLE NAME IS A CONTRACT WITH THE APP, not a choice made here. It must
// equal `APP_DB_ROLE` in server/db/rls-bootstrap.ts, which is what
// drizzle/provision-app-role.ts actually creates. A URL naming a role nobody
// creates fails authentication on every connection.
// tests/unit/infra/app-role-name.test.ts asserts the two spellings match, so
// this line cannot drift silently.
var appDbRole = 'tokenscope_app'
...
// The app-role password, as the provisioning step consumes it
// (TOKENSCOPE_APP_DB_PASSWORD). Written only on an apply that deliberately
// generates one; empty = NO-OP, existing value untouched.
resource secretAppDbPassword 'Microsoft.KeyVault/vaults/secrets@2024-11-01' = if (!empty(appDbPassword)) {
  parent: keyVault
...
// The same credential as a connection URL, for TOKENSCOPE_APP_DATABASE_URL. Two
// secrets and not one because they are consumed at different moments by
// different code: the password provisions the role at boot (as the OWNER), the
// URL is what the runtime pools connect with afterwards (as the role).
resource secretAppDatabaseUrl 'Microsoft.KeyVault/vaults/secrets@2024-11-01' = if (canBuildAppDatabaseUrl) {
  parent: keyVault
  name: 'database-url-app'
  properties: {
    // Identical posture to database-url above — same server, same database,
    // and the same sslmode=verify-full that makes postgres@3.4.9 actually
    // AUTHENTICATE the server (see that secret's comment and drizzle/connect.ts;
    // require/allow/prefer would silently set rejectUnauthorized=false).
    // Only the credentials differ: this one is the non-owner app role.
    value: 'postgresql://${uriComponent(appDbRole)}:${uriComponent(appDbPassword)}@${pgServerFqdn}:5432/tokenscope?sslmode=verify-full'
  }
}
