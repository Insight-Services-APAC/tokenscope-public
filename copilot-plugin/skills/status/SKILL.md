...
> highest-likelihood *silent* failure — it would look "green" if you only checked
...
```bash
node ~/.copilot/plugins/tokenscope/scripts/status.mjs
```
...
```json
{
  ...
  "landed": true,
  "landed_check": { "state": "landed", "last_emission": "2026-06-22T08:14:03Z", "message": "A record landed — ..." },
  "attributed": null,
  "attribution": {
    "state": "unknown",
...
- `emitting: true` → **Emitting.** The credential is valid; sessions will attribute.
  This proves the credential works, not that a record has landed (that's the next check).
...
**2. Landed** (`landed` + `landed_check`) — *landing is necessary, not sufficient:*
...
A genuinely healthy device reads **emitting: true + landed: true + attribution.state:
"attributed"** (i.e. `my_usage` `needs_tagging_count === 0`). **`landed: true` ALONE
is NOT healthy** — it can hide spend landing unbound. `emitting: true` with
`landed: false / unconfirmed` is normal right after setup or a fresh session — give it
a few minutes of ingest lag.
