/*
 * Bundled enrollment secret (Copilot CLI plugin) — PLACEHOLDER in source, injected
 * at marketplace publish. Mirrors plugin/scripts/enrollment-secret.mjs (the Claude
 * half); it is the client half of the emit-on-install gate
 ...
 *
 * NOTE: this is a STANDALONE source file, NOT a sync-generated copy of the Claude
 * module (that one imports nothing and is identical in shape, but the two plugins
 * ship independently and the publish pipeline injects each separately). It is
 * deliberately dependency-free so the standalone copilot-plugin needs no
 * plugin/scripts/* siblings.
 *
 ...
 *       committed). This mirrors how the API base default is baked in elsewhere,
