    ...
    if (!(await sel.count())) {
      ...
      if (!other) bad('NO-TARGET', 'admin users', 'the picker offered no second BU to move to')
        ...
        if (!(await dialog.count())) {
          ...
          } else if (!/would move to|already on|Nothing recorded in that range/.test(spanText)) {
            bad('SPAN-NOT-RENDERED', 'confirmation dialog', 'the check produced no figure')
          } else ok('the dialog renders what would move', /(\$[\d,.]+ of recorded usage[^.]*\.)/.exec(spanText)?.[1] ?? 'zero-state')
